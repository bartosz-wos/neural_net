...
Tensor MoELayer::forward(const Tensor& input) {
    ...
    Tensor gate_values = gating_network_.forward(input); // (batch, num_experts)

    // Dispatch inputs to top-k experts
    auto dispatched = dispatcher_.dispatch(input, gate_values);

    // Run each expert that received a non-empty input
    std::vector<Tensor> expert_outputs(num_experts_ * input.rows, Tensor(1, 0));
    for (size_t e = 0; e < num_experts_; ++e) {
        for (size_t b = 0; b < input.rows; ++b) {
            size_t idx = b * num_experts_ + e;
            if (dispatched[idx].rows > 0) {
                expert_outputs[idx] = experts_[e].forward(dispatched[idx]);
            }
        }
    }
    ...
    for (size_t b = 0; b < input.rows; ++b) {
        ...
        for (size_t i = 0; i < k_; ++i) {
            ...
            const Tensor& out = expert_outputs[b * num_experts_ + expert_idx];
            if (out.rows > 0) {
    ...
    load_balance_loss_ = dispatcher_.load_balance_loss(gate_values,
